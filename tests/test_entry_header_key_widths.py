...
@pytest.mark.parametrize("key_size", [4, 8, 12, 16])
def test_wide_keys_still_use_the_u32_branch(key_size):
    """Inertness, stated as a test. Anything other than 1 or 2 must read
    the id as a u32 exactly as before — those tables have no name header,
    so a different answer would be an unverifiable guess."""
    body = bytes(range(1, 33))
    eid, _name, _pay = _parse_entry_header(body, 0, key_size)
    assert eid == struct.unpack_from("<I", body, 0)[0]
