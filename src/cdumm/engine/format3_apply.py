    ...
    for mod_id, mod_name, json_source, _priority in rows:
        ...
        for target, intents in target_pairs:
            try:
                ...
                # The validator accepts a nested iteminfo path on its ROOT
                # existing in ANY layout CDUMM knows -- but this game runs
                # exactly one of them. Now that the real bytes are here, drop
                # the intents whose root the installed record doesn't carry,
                # and say so. Without this the mod imports clean, reports N
                # intents "ready to apply", and then changes nothing.
                # No-op on a game version that does carry the field.
                kept, layout_dropped = drop_intents_the_layout_cannot_carry(
                    target, validation.supported, vanilla_body, vanilla_header)
                if layout_dropped:
                    reasons = sorted({why for _i, why in layout_dropped})
                    logger.warning(
                        "Format 3 mod '%s' (id=%d): %d of %d intent(s) on %s "
                        "target a field this game's item record does not "
                        "carry; refused rather than silently applying "
                        "nothing. %s",
                        mod_name, mod_id, len(layout_dropped),
                        len(validation.supported), target, reasons[0])
                    if warnings_out is not None:
                        for why in reasons:
                ...
                supported = _expand_match_intents(
                    target, vanilla_body, vanilla_header, kept)
                if not supported:
