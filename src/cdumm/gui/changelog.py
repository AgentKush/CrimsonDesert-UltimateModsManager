...
CHANGELOG = [
    {
        ...
        "version": "3.5.0+community.17",
        "date": "2026-07-18",
        "notes": [
            "<b>Find Culprit works on the native macOS game.</b> The crash-bisection feature spots the game by its running process. On the native macOS Steam build that process is <code>CrimsonDesert_Steam</code>, but CDUMM only looked for the Windows <code>CrimsonDesert.exe</code> — so it never saw the game, treated every bisection round as a crash, and could pin the blame on an innocent mod. It now recognises the macOS executable too (and the Apply-while-running guard does as well). Found via lwjiyuan on GitHub (#299).",
            "<b>Notification text can be read, copied, and is saved to the log.</b> The pop-up messages — including the \"N patches skipped\" summary after an Apply — can now be selected and copied with the mouse, and every notification is also written to CDUMM's log so it lands in a bug report even after the pop-up fades. And in dark mode the notification list is readable again: a faint panel behind each line of text was making it hard to read; that panel now sits only behind the row, not the text. Found via falobos76 on GitHub (#191).",
            "<b>\"Variant pack\" mods (one download, several versions) never dead-end anymore.</b> Some mods bundle several versions of the same tweak in one file — Fat Stacks, for example, ships eight stack-size options (2×, 3×, … up to 999999×). Dropping the zip on the CDUMM window already showed a picker to choose one. But if that pack reached the importer another way — re-importing from source, or a drop that couldn't pre-extract for the picker — you'd hit a dead-end error telling you to drop it on the window. Now every path imports it the same way: as a single mod with one version enabled and the rest one click away on the mod's cog, so it's never a dead end. Found via falobos76 on GitHub (#191).",
            "<b>Mods that ship as a zip with a folder inside now import.</b> DMM's Mod Builder saves each mod as a zip containing a folder of the same long name with the .json inside it (<code>ModName/ModName.json</code>). On Windows, unpacking that doubled ~100-character name under CDUMM's staging folder pushed the path past the 260-character limit Windows still enforces for most apps, so the import failed with a \"No such file or directory\" error — while importing the bare .json worked. CDUMM now collapses that single redundant wrapper folder as it unpacks, keeping the path short, so the zip imports the same as the loose .json. Numbered game-data folders (0036/ and the like) are never collapsed. Found via falobos76 on GitHub (#191).",
            "<b>Equip Everything and the 'anyone can wear anything' mods now actually apply.</b> These clear the tribe and gender restrictions the game stamps on gear, by rewriting the equip-restriction block the July patch moved to the end of each item record. CDUMM learned to read and rewrite that block a couple of builds back (#285), but the apply path still turned those edits away as 'a list this table doesn't have a writer for', so the mod imported and changed nothing. It's wired all the way through now: 'Equip Everything' applies all 2,548 of its item changes plus its 13 equip-slot changes, every targeted restriction is actually emptied, and nothing else in the 6,508-item table moves — verified byte-for-byte against the real mod on the live 1.14.00 game. AXIOM QoL and any other mod that edits these restrictions are covered the same way. (#285)",
            "<b>New: 'no dragon / companion re-summon cooldown' mods now apply.</b> Mods that remove the wait before you can re-summon your dragon mount or a companion set a hidden cooldown field on those characters. CDUMM read every other field on those records but not that one, so the mod imported and quietly did nothing. It's covered now — the cooldown drops to instant on exactly the mount and companion records the mod targets (verified byte-for-byte on the real game), and nothing else moves. Found while testing a DMM Mod Builder preset end to end.",
            "<b>Presets that mix supported and not-yet-supported tweaks now apply the parts CDUMM can do.</b> A Mod Builder preset often bundles several tweaks at once — say bigger inventory + infinite durability + a drop-rate multiplier. If even one tweak used something CDUMM doesn't handle yet (like multiply-a-value), the whole preset was refused at import and none of it applied. CDUMM now applies every tweak it supports and cleanly skips only the ones it can't, telling you which. Tested against a real 9-tweak preset: its inventory, max-stack and infinite-durability tweaks apply byte-for-byte; only the two drop/gimmick multipliers are skipped, each with a note.",
            "<b>Ready for the 1.14.00 game update.</b> When Crimson Desert patches, the game's data files shift position and CDUMM's saved 'vanilla' copy stops lining up — which made 'Convert to a patch-proof mod' fail (with a confusing decode error on some tables) right after an update. CDUMM now notices when the game has changed and reads the current game files directly, so conversion keeps working; if it genuinely can't read a vanilla file it now says so clearly and points you to Settings → Fix Everything instead of crashing. Verified against the live 1.14.00 install. Good news checked at the same time: the game's table formats did not change in 1.14, so your existing patch-proof mods stay valid.",
            "<b>Mods made with DMM's Mod Builder import more fully.</b> DMM's Mod Builder exports the same patch-proof (Format 3) format CDUMM uses, but a few of its shortcuts were being rejected, which made whole presets fail to import. Now supported: 'apply to every item' rules (an empty match, e.g. infinite durability on all items), the 'any of these ids' shorthand DMM writes to hit several records at once, and the inventory storage table — so 'max inventory / bigger storage' presets that raise slot counts on the warehouse, character bag and housing storages now apply. Each verified byte-for-byte against the real game tables. Two of DMM's advanced operators — multiply-a-value and 'every element in a list' on the drop/gimmick tables — aren't covered yet.",
            "<b>New: the 'always-active' stat mods now apply — Direct Movement Speed, Direct Attack Speed and the rest of that class.</b> These popular mods make a stat permanently maxed with no gear required, by rewriting a hidden per-level table inside the game's stat data. CDUMM knew the stat names but never read those record bodies, so the mods (which ship as '.cdmod' packages) imported cleanly and changed nothing. CDUMM now applies them — writing all 16 per-level values in place — and only ever touches the four stats that actually carry that table (movement speed, attack speed, critical rate and DHIT), refusing any other stat so a regular stat can never be corrupted. Verified byte-for-byte on the real 1.13 stat table and against the real Direct Movement Speed x10 mod, end to end.",
            "<b>Cooldown mods on gear now work — including 'No Cooldown'-style item mods.</b> A mod that changes an item's cooldown (like the Great Thief's Gloves 30-minute → 1-second mod) was landing on the wrong bytes, so it did nothing and couldn't be converted to a patch-proof form. On one family of gear the item's cooldown values sit one byte later than CDUMM was reading them, so it read a nonsense number instead of the real cooldown. CDUMM now reads them correctly, and the 'Convert to a patch-proof mod' action handles a cooldown change that spans all three of an item's cooldown fields in one go — verified byte-for-byte, and checked to leave all 6,508 items unchanged where they should be. Found via falobos76 on GitHub (#191).",
            "<b>Byte-offset mods and semantic mods can finally edit the same table together — every change, not most of them.</b> The two mod formats disagree about what a game file is: an older mod says 'at byte 4,192,880, change these bytes', while a newer one has CDUMM rebuild the whole table. The moment a rebuild makes one early record longer, every byte offset after it shifts, and the older mod then writes into the middle of some other item, leaving a file the game refuses to load. CDUMM works out where the bytes moved and rewrites the offsets to match. The ones it couldn't move — a byte offset landing on a record the rebuild had rewritten, where the offset no longer means anything — used to be dropped: on falobos76's list, his socket mods plus 'Infinite Durability' left 76 of 231 durability changes unapplied. Those are now recovered too: CDUMM names the exact item and field each dropped offset meant (looked up against your vanilla files, the one place it still matches) and re-applies it as a field edit on top of the rebuild. Sockets and durability on the same armor both land — all 231 apply, none silently dropped. Found via falobos76 on GitHub (#293, #191).",
            ...
            "<b>Large mods apply in seconds, not minutes.</b> Applying a big item mod could sit for many minutes on the 'composing' step — long enough to look hung. The cause was in how CDUMM reads the 6,500-item table: while it works out the table's layout it tries reading positions that aren't really text, and a garbage length there could make it decode megabytes of nonsense before giving up, thousands of times over. It now rejects an impossible length instantly. Reading the real 1.13 item table dropped from about 26 seconds to about 1, and a 2,000-change mod now composes in under 2 seconds instead of 30 — the output is byte-for-byte identical, just far quicker to produce. Found via falobos76 on GitHub (#191).",
            "<b>When a mod can't be converted to a patch-proof form, CDUMM now tells you why.</b> Converting a byte-offset mod whose change lands in a part of the item record CDUMM hasn't decoded yet is correctly refused — but the reason used to arrive as a pop-up that rendered blank on some setups, so it looked like nothing happened. The explanation now appears in a normal dialog you can read and dismiss. Found via falobos76 on GitHub (#191).",
            "<b>The What's New notes reappear when a build ships new notes under the same version.</b> A community build is sometimes re-released under the same version number with extra fixes; the What's New pop-up only compared version numbers, so those added notes never showed and the exe looked like it carried stale notes. It now notices when the notes themselves change and shows them (you can always reopen them from Settings → About). Found via AgentKush and falobos76 on GitHub (#191).",
            "<b>Under the hood.</b> This build ships with a real-game-table test gate on every change, a check that fails the build if any engine feature is left unreachable, and a linter — added after an internal QA pass found (and fixed) a case where a finished feature had no button to reach it. 2,504 tests pass, including byte-exact stat-table, inventory, character-cooldown and equip-restriction coverage, long-path zip import, variant-pack import, macOS process detection, and other tests that run against a real game install.",
